...
BEGIN
  ...
  FROM pg_tables
  WHERE schemaname = 'public'
  AND tablename IN ('query_performance_log', 'system_health_metrics', 'security_audit_logs', 'maintenance_logs')
  
  UNION ALL
  
  -- Test 12: Automation functions
  SELECT
    'Automation'::TEXT,
...
BEGIN
  ...
  -- Test 3: Agency statistics (if materialized view exists)
...
BEGIN
  ...
  FROM comprehensive_schema_validation()
  WHERE validation_category = 'Data Quality' AND status = 'FAIL';
...
BEGIN
  ...
  WHERE NOT EXISTS (
    ...
    WHERE jobname = 'check-performance-alerts'
  )
...
FROM pg_proc
WHERE proname IN ('cleanup_old_data', 'refresh_materialized_views', 'optimize_database_performance', 'run_data_consistency_checks', 'collect_system_health_metrics');
